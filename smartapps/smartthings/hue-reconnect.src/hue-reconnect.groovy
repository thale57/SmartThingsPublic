...
Map bulbsDiscovered() {
    ...
    bulbs = state.bulbs // Tracy: required to make it pull the bulbs
    def bulbmap = [:]
    if (bulbs instanceof java.util.Map) {
        ...
        bulbs.each {
        	log.trace "Adding ${it.value.name} to old bulb list"
            def value = "${it.name}"
...
Map scenesDiscovered() {
    ...
    if (scenes instanceof java.util.Map) {
        ...
        scenes.each {
            if (it.value.id == sceneTime?."${it.value.name.minus(~/ on \d+/)}"?.id)// && it.value.name ==~ /.* on \d+$/) //Tracy: commented out second half
            {
    ...
    if (state.inItemDiscovery) {
        ...
        object.each { k,v ->
            if (v instanceof Map) {
                ...
                if(v.type == "Extended color light" || v.type == "Color light" || v.type == "Dimmable light" || v.type == "Color temperature light") { // Tracy: added "Color temperature light"
        ...
        if(!d) {
            ...
            if (bulbs instanceof java.util.Map) {
                ...
                if (newHueBulb) {
                    ...
                    // Added by Tracy: Need to handle the "Color temperature light" is a Hue White Ambiance Bulb
                    ...
                    {
                    	log.debug "trying to add the color temp light"
